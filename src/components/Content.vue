<template>
  <div class="bb-content">
    <div class="sszk" style="width:100%">
      ...
      <el-switch v-model="$root.isState.isWordHint" active-color="#13ce66" inactive-color="#dd1b54">
      </el-switch>
    </div>
    <div class="sszk" style="width:100%">
      ...
      <el-tag style="margin:3px;" v-if="$root.userInfo.userId !== ''">
        今:{{$root.currentPage.currentDaySize}}/总:{{$root.userInfo.countSize}}
      </el-tag>
    </div>
    <div class="sszk" style="width:100%">
      <el-button type="primary" id="anew" icon="el-icon-s-operation" @click="$root.isState.isDrawer = true"></el-button>
    ...
    <el-progress style="margin-bottom:5px;"
    ...
    <div class="gdk" :class="[!$root.isState.isWordHint?'mention':'']" id="gdk">
         ...
         <span class="spanText" :id="'gd_'+($root.currentPage.startStrLength+i)"
               v-for="(item,i) in $root.textJson.hintAry[$root.isState.isWordHint?'yesHint':'noHint']"
               :key="i" :class="[ item.hasOwnProperty('ys') ? item.ys : '' ]"
               :style="[
					...
					{{item.val}}
					<font style="font-style:normal;font-variant: initial;" v-show="
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      ['$root.textJson.dealWithText']() { // 计算击键、打词率、退格
        this.$root.sszk.zjs += '-'
        ...
        if (this.$root.sszk.previousBm.length !== 0 && ciLenth === this.$root.sszk.previousBm.substr(0, this.$root.sszk.previousBm.length - 1)) this.$root.sszk.backSpace += 1 // 计算退格
        if (/^[a-z]+$/g.test(ciLenth)) this.$root.sszk.previousBm = ciLenth // 计算退格,上一个打的abc，下一个打的ab，通过这规律判断
        if (this.$root.gdqJson.withStr[currentTextL + 1] === this.$root.textJson.dealWithText[currentTextL + 1]) { // 若打的字正确，直接射入
        ...
        } else if (!/[a-z]/g.test(this.$root.textJson.dealWithText.substr(currentTextL)) || ciLenth.length > 4) { // 如果打的不正确，并且打的是英文以外的字符，就射入
          this.$root.textJson.tR = this.$root.textJson.dealWithText
          this.$root.sszk.previousBm = ''
        }
      },
      async ['$root.textJson.tR']() {
        ...
        if (this.$root.textJson.tR.length < this.$root.textJson.tRLength) { // 回改了哦
          this.$root.sszk.hg++
          const kt = this.$root.textJson.tR.length
          for (let i in '1'.repeat(10)) { // 这里也要修改下哈，泥马了个憋
        ...
        this.$root.textJson.hintAry.isCorrectAry.forEach((e, i) => { // 将未输入的那部分转成黑色的字体
        ...
        this.$root.textJson.tR.slice(tRStartLength, tRStartLength + 50).split('').forEach((e, i) => { // 每改变一次文字，循环判断一遍是否输入正确
        ...
        if (this.$root.textJson.tR.length === 0) { // 归零
          this.anew()
          this.countSd()
          this.toast('听说你是重打王？')
        }
        if (this.$root.gdqJson.startTime === null || this.$root.textJson.tRLength === 0) { // 判断是不是刚开始打
        ...
        if (this.$root.textJson.tR.length > 0) { // 开始跟打
          this.$root.sszk.start = (new Date().getTime() - this.$root.gdqJson.startTime - this.$root.gdqJson.pauseTime) / 1000 // 当前所跟打了的时间 , 其中还减去了暂停的时间
          this.countSd()
        }

        if (this.$root.textJson.tR.length >= this.$root.gdqJson.withStr.length
          && this.$root.textJson.tR[this.$root.gdqJson.withStr.length - 1] === this.$root.gdqJson.withStr[this.$root.gdqJson.withStr.length - 1]) { // 跟打完成
          this.$root.currentPage.currentPageNum = 0
          this.$root.textJson.tR = this.$root.textJson.tR.substr(0, this.$root.gdqJson.withStr.length)
          this.$root.gdqJson.endTime = new Date().getTime()
          this.$root.sszk.start = (this.$root.gdqJson.endTime - this.$root.gdqJson.startTime - this.$root.gdqJson.pauseTime) / 1000
          this.countSd()
          this.addCountSize()
          ...
          if (countTextSpeed !== '错字0') {
            str = `第${this.$root.gdqJson.severnNum}段 速度${countTextSpeed[1]} 击键${this.$root.sszk.jj} 码长${this.$root.sszk.mc} 回改${this.$root.sszk.hg} 退格${this.$root.sszk.backSpace} 键准${keyAccurate === -Infinity ? 100.00 : keyAccurate.toFixed(2)}% ${this.$root.isState.isWordHint ? '理论码长' + this.$root.gdqJson.llmc : ''} 打词${wordRate}% ${countTextSpeed[0]}[错1罚5] 字数${this.$root.gdqJson.withStr.length} 耗时${this.$root.sszk.start}s 个签:${this.$root.userInfo.sign}   -BB打字机`
          } else {
            str = `第${this.$root.gdqJson.severnNum}段 速度${this.$root.sszk.sd} 击键${this.$root.sszk.jj} 码长${this.$root.sszk.mc} 回改${this.$root.sszk.hg} 退格${this.$root.sszk.backSpace} 键准${keyAccurate === -Infinity ? 100.00 : keyAccurate.toFixed(2)}% ${this.$root.isState.isWordHint ? '理论码长' + this.$root.gdqJson.llmc : ''} 打词${wordRate}% 错字0 字数${this.$root.gdqJson.withStr.length} 耗时${this.$root.sszk.start}s 个签:${this.$root.userInfo.sign}   -BB打字机`
          }
    ...
    methods: {
      changeScrollBar() { // 控制滚动条的高度
        var gdk = document.getElementById('gdk')
        if (this.$root.textJson.tR.length < 50) return
        gdk.scrollTop = ((gdk.scrollHeight - gdk.clientHeight) / this.$root.gdqJson.withStr.length) * this.$root.textJson.tR.length
      },
      addCountSize() {
        this.$axios.post(`/savaCountSize`, {
      ...
      share() { // 分享发文
        ...
        const str = `${withStr}\n-----第${this.$root.gdqJson.severnNum}段`
        androids.setPasteText(str)
        androids.sendMsgQQ(str)
      },
      anew() { // 重打
        ...
        this.$root.textJson.tR = ''
        this.$nextTick(() => { // 页面渲染完后，再做下面的步骤，别问为什么，问就是不告诉你
          this.$root.gdqJson.startTime = null
      ...
      countErrorText() { // 返回错字及错字后的时速
        let errorNum = 0
        let errorAry = []
        this.$root.textJson.tR.split('').forEach((e, i) => { // 每改变一次文字，循环判断一遍是否输入正确
        ...
        if (e.hasOwnProperty('ys')) {
          ...
          bmLength.forEach(ee => {
            if (e.ys === `bm${ee}` && e.val.length > 4) {
              if (!this.$root.wordJson.analysisJson.hasOwnProperty(`${ee}码长词`)) this.$root.wordJson.analysisJson[`${ee}码长词`] = []
              this.$root.wordJson.analysisJson[`${ee}码长词`].push(e.val)
            }
            if (e.ys === `bm1` && e.val.length > 4) {
              if (!this.$root.wordJson.analysisJson.hasOwnProperty(`一简长词`)) this.$root.wordJson.analysisJson[`一简长词`] = []
              this.$root.wordJson.analysisJson[`一简长词`].push(e.val)
            } else if (e.ys === `bm1` && e.val.length === ee) {
              if (!this.$root.wordJson.analysisJson.hasOwnProperty(`一简${ee}字词`)) this.$root.wordJson.analysisJson[`一简${ee}字词`] = []
      ...
      hintWordMention() { // 词提编码遍历显示
        if (!this.$root.isState.isWordHint) return  // 如果没开启词提模式直接返回不让他执行下去
        console.log(this.$root.textJson.hintAry.ckHintAry)
      ...
      async sendArticle() { // 读文
        ...
        if (this.$root.isState.isReadTextWay === '1') {
          let pro = new Promise(async resolve => {
            ...
            if (this.$root.gdqJson.withStr === '') { // 拿不到文
      ...
      async start() { // 读取一波词提
        return new Promise(async resolve => {
          ...
          this.$root.gdqJson.withStr.split('').forEach((e, i) => {
            ...
            this.$root.textJson.hintAry.noHint.push({ // 不开词提的
              val: e,
              length: 1,
              index: 1,
              i_: i
          ...
          let i = 0
          this.$root.textJson.hintAry.ckHintAry.forEach((e, ii) => { // 来，泥马，跟哥横个看看？来啊，马勒戈壁
      ...
      verifCurrentIsYes() { // 验证本页的对错
        this.$nextTick(() => {
          this.$root.gdqJson.withStr.slice(this.$root.currentPage.startStrLength, this.$root.currentPage.startStrLength + this.$root.currentPage.onePageSize).split('').forEach((e, i) => { // 每改变一次文字，循环判断一遍是否输入正确
