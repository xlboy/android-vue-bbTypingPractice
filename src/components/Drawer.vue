<template>
  <div class="bb-drawer">
    <el-drawer
      class="gdq-drawer"
      :show-close="false"
      :visible.sync="$root.isState.isDrawer"
      direction="rtl"
    ...
    >
      <el-button
        ...
        circle
        onclick="androids.webViewReload()"
      ...
      <div class="head-box">
        ...
        <div class="jz">
          <el-link
            type="primary"
            class="login-text"
            @click="$root.isState.isRegLoginDialog = true"
          ...
          <el-link
            ...
            @click="logout()"
            v-if="$root.userInfo.userId !== ''"
          >{{$root.userInfo.username}}</el-link>
          <el-link
            type="danger"
            style="margin-left:3px"
            class="login-text"
            @click="$root.isState.isBindQQ = true"
      ...
      <div class="jz word-box">
        <el-tag
          type="success"
          style="width: 100%;"
          @click="$root.isState.isSign = true"
        >{{$root.userInfo.sign}}</el-tag>
      </div>
      <div class="jz word-box">
        <el-button
          type="primary"
        ...
        ></el-button>
        <el-select
          v-model="$root.userInfo.currentWordId"
          @change="wordUpdate"
          no-data-text="无词库数据.."
          placeholder="选择词库"
      ...
      <div class="jz colorWord">
        <el-link class="flex" :style="{color : $root.userInfo.twoWord}" href="#">
          二码
          <el-color-picker
            size="mini"
            color-format="rgb"
            :show-alpha="true"
            v-model="$root.userInfo.twoWord"
          ></el-color-picker>
        </el-link>
        <el-divider direction="vertical"></el-divider>
        <el-link class="flex" :style="{color : $root.userInfo.threeWord}" href="#">
          三码
          <el-color-picker
            size="mini"
            color-format="rgb"
            :show-alpha="true"
            v-model="$root.userInfo.threeWord"
          ></el-color-picker>
        </el-link>
        <el-divider direction="vertical"></el-divider>
        <el-link class="flex" :style="{color : $root.userInfo.fourWord}" href="#">
          四码
          <el-color-picker
            size="mini"
            color-format="rgb"
            :show-alpha="true"
            v-model="$root.userInfo.fourWord"
          ></el-color-picker>
        </el-link>
        <el-divider direction="vertical"></el-divider>
        <el-link class="flex" :style="{color : $root.userInfo.fourC}" href="#">
          >=四字词
          <el-color-picker
            size="mini"
            color-format="rgb"
            :show-alpha="true"
            v-model="$root.userInfo.fourC"
          ></el-color-picker>
        </el-link>
      </div>
    </el-drawer>
...
export default {
  ...
  watch: {
    ['$root.userInfo.twoWord']() {
      // 颜色改变
      this.updateWordColor()
    },
    ['$root.userInfo.threeWord']() {
      // 颜色改变
      this.updateWordColor()
    },
    ['$root.userInfo.fourWord']() {
      // 颜色改变
      this.updateWordColor()
    },
    ['$root.userInfo.fourC']() {
    ...
    readDefaultWord() {
      ...
      return new Promise(resolve => {
        this.$axios.post(`/getDefaultWord`, {}).then(response => {
          ...
          this.$root.wordJson.optionAry = [ this.$root.wordJson.defaultAry,this.$root.wordJson.userAry]
        })
      })
    },
    wordUpdate(e) {
